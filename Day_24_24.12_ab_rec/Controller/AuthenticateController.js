...
class Auth 
{
    ...
    async create_user(req,res)
     ...
     {
        req.session.destroy()
        res.render('Login',{mesg:"Logout Successfully"})
     }
}
                    ...
                    if(req.body.confpassword == req.body.newpassword)
                    {
                       ...
                       const hpassw=await bcrypt.hash(req.body.newpassword,bcrpyt_salt)
                        const data={
                            ...
                            if (err)
                            {
                                res.render('Pchangepassword',{mesg:" Error in While Checking"})
                            }
                            else
                            {        
                                  const dbpass=result[0].password 
      ...
      Patients_Profile_Complete(req,res)
      {
           if(!req.session.patient_email)
            ...
            const data = {
                    ...
                    Dhistoty:req.body.dhistoty,
                    Profile_Photo:req.file.filename
                }
                    usermodel.Patient_Profile_update(data,(err)=>{
                            if(err)
                            {
                                res.render('Login',{mesg:"Error in Profile Completion Contact Admin"+err})
                            }
                            else
                            {
