const mysql=require('../Databsae/myconnect.js')
class Student{
    Add_student(req,res)
    {
        if(req.method=="GET")
    ...
    update_Student_record(req,res)
    {
        const {name,roll,branch,address}=req.body;
       
        const q =`update student set name ='${name}', branch='${branch}', address= '${address}' where roll='${roll}'`;
        mysql.query(q,(err)=>{
            if(err){
